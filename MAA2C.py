...
class MAA2C(Agent):
    ...
    def __init__(self, env, n_agents, obs_shape_n, act_shape_n,
        ...
        if self.training_strategy == "cocurrent":
            ...
            for i in range(self.n_agents):
                ...
                self.critics.append(CriticNetwork(whole_critic_state_dim, whole_critic_action_dim, self.critic_hidden_size, 1))
    ...
    def interact(self):
        ...
        for i in range(self.roll_out_n_steps):
            ...
            for agent_id in range(self.n_agents):
                actions.append(index_to_one_hot(action[agent_id], self.act_shape_n[agent_id]))
    ...
    def _softmax_action(self, state):
        ...
        for i in range(self.n_agents):
            np.vstack((softmax_action,self.act_shape_n[i]))
    ...
    def value(self, state, action):
        ...
        for i in range(self.n_agents):
            whole_critic_state_dim += self.obs_shape_n[i]
            whole_critic_action_dim += self.act_shape_n[i]
        whole_state_var = state_var.view(-1, whole_critic_state_dim)
